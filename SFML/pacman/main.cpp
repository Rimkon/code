...
int main() {
	srand(time(0));
	RenderWindow window(VideoMode(width * ts, height * ts), "Maze!");
	...
	while (window.isOpen())
	{
		...
		for (int i = 0; i < height; i++)
			for (int j = 0; j < width; j++) {
				...
				if (TileMap[i][j] == 'C') plat.setTextureRect(IntRect(ts * int(p.frame), ts * p.rotate, ts, ts));
				...
				if (TileMap[i][j] == '2') plat.setTextureRect(IntRect(ts * 5, ts * en.rotate[1], ts, ts));
				if (TileMap[i][j] == '3') plat.setTextureRect(IntRect(ts * 5, ts * en.rotate[2], ts, ts));
				if (TileMap[i][j] == '4') plat.setTextureRect(IntRect(ts * 5, ts * en.rotate[3], ts, ts));
				if (TileMap[i][j] == 'B')
					continue;
