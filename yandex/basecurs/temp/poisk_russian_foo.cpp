...
typedef string строка
typedef return вернуть
typedef int число
                                                  
строка // --------------------------------------------------------- чтение строки ----
ЧитатьСтроку() {                                               //  Вход: поток символов
                                                           // Выход: строка
        строка стр;
...
число
ЧитатьСтрокуИЧисло() { // ---------------------------------- чтение числа и строки ---
                                                           //    Вход: запрос числа.
        число результат= 0;                                    // Зависит: ЧитатьСтроку()
...
РазбитьСтрокуНаСлова  ( const string& text ) {                   //  Вход: строка слов
...
ДобавитьДокумент  ( vector< pair<int, vector<string>>>  &documents,       
...
main() {
                                                           // стоп слова 
    const     string   stop_words_joined = ЧитатьСтроку();
    const set<string>  stop_words        = РазбитьСтрокуНаСлова(stop_words_joined);

                                                           // наполнение документа 
                                                           // Считываем документы
    vector<pair<int, vector<string>>> documents;
    const int document_count = ЧитатьСтрокуИЧисло();

    for (int document_id = 0; document_id < document_count; ++document_id) {
        ДобавитьДокумент(documents, stop_words, document_id, ЧитатьСтроку());
    }

                                                           // искомая строка
    const string query = ЧитатьСтроку();
