...
set<string> ParseQuery(const string& text, const set<string>& stop_words) {
  ...
  vector<string> vs = SplitIntoWordsNoStop (text, stop_words);
    // Напишите код функции
    query_words.insert (vs.begin(), vs.end());
...
vector<int> FindDocuments(const vector<vector<string>>& documents, const set<string>& stop_words,
    ...
    int id = 0;
    set<string> no_stop_word = ParseQuery(query, stop_words);
    
    for (const vector<string> vect_str: documents)
    {
        if  ( MatchDocument(vect_str, no_stop_word))
        {
